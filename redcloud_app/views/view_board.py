...
from redcloud_app.views.view_templates import LabelH1, LabelH2, HR, BoxView, LabelH3
...
class BoardScreen(BoxView):
    ...
    def populate_screen(self, issue_id):
        ...
        self.issue_activities = toga.Selection(style=Pack( flex=0.5, margin=5),
            items=list(map(lambda dl: f'{dl[0]} - {dl[1]}', project['time_entry_activities'])))
        self.issue_status = toga.Selection(items=list(map(lambda dl: f'{dl[0]} - {dl[1]}', redmine_status)), style=Pack(flex=0.5, margin=5))

        box_kpis.add(toga.Box(style=Pack( gap=5, direction=ROW), children=[self.issue_status,self.issue_activities]))  # Scrollable
